...
from django.contrib import messages #changes
from django.conf import settings #changes
from django.core.mail import send_mail #changes
    ...
    if request.method == 'GET':
        ...
        if request.user.user_type == 'Patient':
            ...
            if patient_obj:
                ...
                em=[patient_obj.doctor.user.email] #in send_mail function to email must be a list or tuple
                body="{} is now your patient! ".format(patient_obj.user.username)
                # mel=EmailMessage("New Patient",body,to=[em])
                # mel.send()
                # -----changes send_mail func------
                from_email = settings.EMAIL_HOST_USER
                send_mail('New Patient', body, from_email, em, fail_silently=True )
                # fail_silently prevents the error from being raised
                patient_obj.save()
                return HttpResponseRedirect(reverse('mainapp:dashboard'))
            else:
                return HttpResponseRedirect(reverse('mainapp:reguser'))
...
def mail(request):
    if request.method == 'POST':
        if request.user.user_type == 'Patient':
            pat = Patient.objects.get(user=request.user)
            docEmail = [pat.doctor.user.email] #in send_mail function to email must be a list or tuple
            body = request.POST.get('body')
            # email = EmailMessage('Medical Report', body, to=[docEmail])
            # email.send()
            # -----changes send_mail func------
            from_email = settings.EMAIL_HOST_USER
            send_mail('Medical Report from '+request.user.username, body, from_email, docEmail )
            # fail_silently prevents the error from being raised
            return HttpResponseRedirect(reverse('mainapp:dashboard'))
        if request.user.user_type == 'Doctor':
            doc = Doctor.objects.get(user=request.user)
            patEmail = [request.POST.get('emailauth')] #in send_mail function to email must be a list or tuple
            body = request.POST.get('body')
            # email = EmailMessage('Medical Report: Reply', body, to=[patEmail])
            # email.send()
            # -----changes send_mail func------
            from_email = settings.EMAIL_HOST_USER
            send_mail('Medical Report: Reply from '+request.user.username, body, from_email, patEmail )
            # fail_silently prevents the error from being raised
            return HttpResponseRedirect(reverse('mainapp:dashboard'))
    if request.method == 'GET':
        if request.user.user_type == 'Patient':
